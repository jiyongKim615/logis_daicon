...
def get_larger_range_first_last_utils(v1, df, num=3):
    new_df = df.copy()
    num_lst = []
    num_index = df.columns.tolist().index(v1)
    for i in range(len(df)):
        str_num_first = str(df.iloc[i, num_index])[:num]
        str_num_last = str(df.iloc[i, num_index])[-num:]
        # print(str_num)
...
def get_larger_range_first_utils(v1, df, num):
    ...
    for i in range(len(new_df)):
        str_num = str(new_df.iloc[i, num_index])[:num]
        # print(str_num)
